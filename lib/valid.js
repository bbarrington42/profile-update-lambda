...
exports.validateCSV = (arg, config) => {
    ...
    if (hasDups(header.slice(0))) {
        errors.push('Header elements must be unique');
        return Promise.reject(errors);
    }
    ...
    const passedOptional = _.difference(header, requiredHeaders);
    const difference = _.difference(passedOptional, optionalHeaders);
    console.log(`passedOptional: ${passedOptional}`);
    ...
    if (_.isEmpty(errors)) {
        ...
        _.forEach(lines, (line, row) => {
            ...
            if (header.length !== values.length) {
                errs.push(`Row ${row + 2} contains the wrong number of values - expected: ${header.length}, actual: ${values.length}`);
            } else {
