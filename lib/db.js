...
exports.doUpdateBeverage = (conn, header, line) => {
    const update = sql.beverageUpdate(header, line);
    console.log(`Beverage update: ${update}`);

    return conn.query(update).then(r => {
        console.log(`Update returned: ${JSON.stringify(r)}`);
        if (r.affectedRows === 0) return Promise.reject('Update failed'); else return Promise.resolve()
    }).catch(err => Promise.reject(err));
};
...
exports.run = (input, config, dbOperation) => {
    ...
    const loop = (conn, input) => {
        ...
        ++total;
        return dbOperation(conn, header, input.shift()).then(_ => {
            ++success;
